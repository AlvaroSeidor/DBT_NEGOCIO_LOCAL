    ...
    pre_hook=[
        ...
        using (
            select
                ...
                HASH(TIMESTAMP, CODE, NAME, INTERNET_ADDRESS, ACCOUNT_NO, FACTOR_CONVERSION_VOL_PESO, PHONE_NO, FAX_NO, E_MAIL, VENDOR_NO, LABEL_TYPE, CATEGORIA_VEHICULO, TONELAJE_VEHICULO, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('shipping_agent_batch') }}
        ) s
...
{% set src_ref_coopecarn = source('bronze_coopecarn', 'COOPECARN_SHIPPING_AGENT') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('shipping_agent_batch_coopecarn') }} v
