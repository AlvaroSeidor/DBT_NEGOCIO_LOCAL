    ...
    pre_hook=[
        ...
        using (
            select
                ...
                HASH(TIMESTAMP, CODE, DESCRIPTION, DEF_GEN_PROD_POSTING_GROUP, DEF_INVENTORY_POSTING_GROUP, DEF_TAX_GROUP_CODE, DEF_COSTING_METHOD, DEF_VAT_PROD_POSTING_GROUP, ITEM_TRACKING_CODE, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('item_category_batch') }}
        ) s
...
{% set src_ref_coopecarn = source('bronze_coopecarn', 'COOPECARN_ITEM_CATEGORY') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('item_category_batch_coopecarn') }} v
