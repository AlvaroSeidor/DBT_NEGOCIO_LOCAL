    ...
    pre_hook=[
        ...
        using (
            select
                ...
                HASH(TIMESTAMP, CODE, DESCRIPTION, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('presentation_batch') }}
        ) s
...
{% set src_ref_coopecarn = source('bronze_coopecarn', 'COOPECARN_PRESENTATION') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('presentation_batch_coopecarn') }} v
