{{ config(
    ...
    pre_hook=[
        ...
        using (
            select
                CODE, tec_des_empresa,
                HASH(CODE, TIMESTAMP, DESCRIPTION, DEFAULT_NOS, MANUAL_NOS, DATE_ORDER, ML_BAJAR_SGI, ML_BLOQUEA_ALBARANES, SII_RECTIFICATIVE_INVOICE, SII_SPECIAL_TYPE, SERIE_ORMA, SERIE_SGI, GS1, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('no_series_batch') }}
        ) s
        on t.tec_cod_vigencia = 1
            and t.CODE is not distinct from s.CODE and t.tec_des_empresa is not distinct from s.tec_des_empresa
...
{% set src_ref_delisano = source('bronze_delisano', 'V_DL_SERIES') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('no_series_batch_delisano') }} v
                  where v.tec_id_ingesta = l.tec_id_ingesta
              );
{% set src_ref_fricafor = source('bronze_fricafor', 'FRICAFOR_NO_SERIES') %}
            update {{ log_ref }} l
            set tec_ts_integracion_b = current_timestamp()
            where l.tec_ts_integracion_b is null
              and l.start_watermark <> l.end_watermark
              ...
              and exists (
                  select 1
                  from {{ ref('no_series_batch_fricafor') }} v
                  where v.tec_id_ingesta = l.tec_id_ingesta
              );"
    ]
) }}
...
with source as (
    select
        ...
        date_order, 
        ml_bajar_sgi, 
        ml_bloquea_albaranes, 
        sii_rectificative_invoice, 
        sii_special_type, 
        serie_orma, 
        serie_sgi, 
        gs1, 
...
{% if is_incremental() %}
left join {{ this }} t
    on  t.CODE is not distinct from s.CODE and t.tec_des_empresa is not distinct from s.tec_des_empresa
