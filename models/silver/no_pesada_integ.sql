{{ config(
    ...
    pre_hook=[
        ...
        on t.tec_cod_vigencia = 1
            and t.NO_PESADA is not distinct from s.NO_PESADA and t.tec_des_empresa is not distinct from s.tec_des_empresa
...
{% set src_ref_fricafor = source('bronze_fricafor', 'FRICAFOR_NO_PESADA') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('no_pesada_batch_fricafor') }} v
...
{% if is_incremental() %}
left join {{ this }} t
    on  t.NO_PESADA is not distinct from s.NO_PESADA and t.tec_des_empresa is not distinct from s.tec_des_empresa
    and t.tec_cod_vigencia    = 1
where t.NO_PESADA is null
