{{ config(
    ...
    pre_hook=[
        ...
        using (
            select
                CUSTOMER_NO, tec_des_empresa,
                HASH(TIMESTAMP, CUSTOMER_NO, CUSTOMER_NAME, COMPANY_CODE, COMPANY_NAME, POLICY_NO, ACTIVE, POLICY_AMOUNT, GRANTED_AMOUNT, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('credito_clientes_batch') }}
        ) s
        on t.tec_cod_vigencia = 1
            and t.CUSTOMER_NO is not distinct from s.CUSTOMER_NO and t.tec_des_empresa is not distinct from s.tec_des_empresa
...
{% set src_ref_delisano = source('bronze_delisano', 'V_DL_CREDITO_CLIENTES') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('credito_clientes_batch_delisano') }} v
...
{% if is_incremental() %}
left join {{ this }} t
    on  t.CUSTOMER_NO is not distinct from s.CUSTOMER_NO and t.tec_des_empresa is not distinct from s.tec_des_empresa
