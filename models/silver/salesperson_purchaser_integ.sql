...
{% set src_ref_coopecarn = source('bronze_coopecarn', 'COOPECARN_SALESPERSON_PURCHASER') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('salesperson_purchaser_batch_coopecarn') }} v
                  where v.tec_id_ingesta = l.tec_id_ingesta
              );
{% set src_ref_fricafor = source('bronze_fricafor', 'FRICAFOR_SALESPERSON_PURCHASER') %}
            update {{ log_ref }} l
            set tec_ts_integracion_b = current_timestamp()
            where l.tec_ts_integracion_b is null
              and l.start_watermark <> l.end_watermark
              ...
              and exists (
                  select 1
                  from {{ ref('salesperson_purchaser_batch_fricafor') }} v
                  where v.tec_id_ingesta = l.tec_id_ingesta
              );"
    ]
) }}
