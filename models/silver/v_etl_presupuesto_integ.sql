...
{% set src_ref_coopecarn = source('bronze_coopecarn', 'V_ETL_PRESUPUESTO_COOPECARN') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('v_etl_presupuesto_batch_coopecarn') }} v
                  where v.tec_id_ingesta = l.tec_id_ingesta
              );
{% set src_ref_delisano = source('bronze_delisano', 'RAW_GC_DP_PRESSUPOST') %}
            update {{ log_ref }} l
            set tec_ts_integracion_b = current_timestamp()
            where l.tec_ts_integracion_b is null
              and l.start_watermark <> l.end_watermark
              ...
              and exists (
                  select 1
                  from {{ ref('v_etl_presupuesto_batch_delisano') }} v
                  where v.tec_id_ingesta = l.tec_id_ingesta
              );
{% set src_ref_fricafor = source('bronze_fricafor', 'V_ETL_PRESUPUESTO_FRICAFOR') %}
            update {{ log_ref }} l
            set tec_ts_integracion_b = current_timestamp()
            where l.tec_ts_integracion_b is null
              and l.start_watermark <> l.end_watermark
              ...
              and exists (
                  select 1
                  from {{ ref('v_etl_presupuesto_batch_fricafor') }} v
                  where v.tec_id_ingesta = l.tec_id_ingesta
              );"
    ]
) }}
