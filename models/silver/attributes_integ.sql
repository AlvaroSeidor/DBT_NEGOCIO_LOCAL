    ...
    pre_hook=[
        ...
        using (
            select
                ...
                HASH(TIMESTAMP, TABLE_ID, TABLENAME, CODE, DESCRIPTION, ATTRIBUTEID, ATTRIBUTEIFID, ATTRIBUTENAME, ATTRIBUTEVALUE, UOM, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('attributes_batch') }}
        ) s
...
{% set src_ref_delisano = source('bronze_delisano', 'V_DL_ATTRIBUTES') %}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('attributes_batch_delisano') }} v
