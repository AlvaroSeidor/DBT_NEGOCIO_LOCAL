{{ config(
    ...
    pre_hook=[
        ...
        using (
            select
                tec_des_empresa,
                HASH(TIMESTAMP, FIELD_CODE, CODE, DESCRIPTION, HEADER_LEVEL_CODE, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('item_classification_batch') }}
        ) s
    ...
    post_hook=[
        "{% set log_ref = source('support_param', 'INSERT_LOGS') %}{% set asociados = var('empresas', []) %}{% for a in asociados %}
            {% set src_ref = source('bronze_' ~ a, 'COOPECARN_ITEM_CLASSIFICATION') %}
            -- Sella ÚNICAMENTE el batch (tec_id_ingesta) que está procesando la view {{ ref('item_classification_batch_' ~ a) }}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('item_classification_batch_' ~ a) }} v
