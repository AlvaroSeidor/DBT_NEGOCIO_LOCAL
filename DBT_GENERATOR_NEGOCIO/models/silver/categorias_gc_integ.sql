{{ config(
    ...
    pre_hook=[
        ...
        using (
            select
                tec_des_empresa,
                HASH(TIMESTAMP, CODCLASIF, GAMA, NOMGAMA, CATEGORIA, NOMCATEGORIA, FAMILIA, TEC_DES_EMPRESA, TEC_ID_INGESTA, TEC_TS_INGESTA, TEC_TS_STAGING, TEC_TS_INTEGRACION_B) AS tec_hash,
                date(tec_ts_ingesta) as tec_fec_inicio
            from {{ ref('categorias_gc_batch') }}
        ) s
    ...
    post_hook=[
        "{% set log_ref = source('support_param', 'INSERT_LOGS') %}{% set asociados = var('empresas', []) %}{% for a in asociados %}
            {% set src_ref = source('bronze_' ~ a, 'V_DL_CATEGORIAS_GC') %}
            -- Sella ÚNICAMENTE el batch (tec_id_ingesta) que está procesando la view {{ ref('categorias_gc_batch_' ~ a) }}
            update {{ log_ref }} l
            ...
            where l.tec_ts_integracion_b is null
              ...
              and exists (
                  select 1
                  from {{ ref('categorias_gc_batch_' ~ a) }} v
