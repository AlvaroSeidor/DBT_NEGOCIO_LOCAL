...
{% set src_ref = source('bronze_coopecarn', 'COOPECARN_ITEM_CLASSIFICATION') %}
...
src as (
    select
        ...
        {{ std_cast('"FIELD_CODE"', 'INTEGER') }}                              as FIELD_CODE,
        {{ std_cast('"CODE"', 'VARCHAR') }}                                    as CODE,
        {{ std_cast('"DESCRIPTION"', 'VARCHAR') }}                             as DESCRIPTION,
        {{ std_cast('"HEADER_LEVEL_CODE"', 'VARCHAR') }}                       as HEADER_LEVEL_CODE,
